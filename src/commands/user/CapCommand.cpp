...
bool CapCommand::execute(User* user, const std::vector<std::string>& params, UserManager* userManager)
{
    if (params.empty())
	{
        ...
        std::string error = ":" + userManager->getServerName() + " " + std::string(ERR_NEEDMOREPARAMS) + " " + nick + " CAP :Not enough parameters\r\n";
	...
	{
        std::string nick = user->getNickname().empty() ? "*" : user->getNickname();
        std::string error = ":" + userManager->getServerName() + " " + std::string(ERR_UNKNOWNCOMMAND) + " " + nick + " " + subcommand + " :Unknown command\r\n";
        userManager->sendMessage(user, error);
        return false;
    }
...
{
    std::string nick = user->getNickname().empty() ? "*" : user->getNickname();
    std::string caps = ":" + userManager->getServerName() + " CAP " + nick + " LS :\r\n";
...
{
    std::string nick = user->getNickname().empty() ? "*" : user->getNickname();
    std::string caps = ":" + userManager->getServerName() + " CAP " + nick + " LIST :\r\n";
    userManager->sendMessage(user, caps);
}
...
void CapCommand::handleReq(User* user, const std::vector<std::string>& caps, UserManager* userManager)
{
    ...
    std::string response = ":" + userManager->getServerName() + " CAP " + nick + " NAK :" + capList + "\r\n";
